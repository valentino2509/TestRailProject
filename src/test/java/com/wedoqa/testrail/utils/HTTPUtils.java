package com.wedoqa.testrail.utils;
...
//        log.trace("Attempting to get {}", completeUrl);
...
//                log.trace("Adding header [{}: {}]", key, value);
    ...
    private HttpURLConnection submitRequestFromConnection(HttpURLConnection connection) throws IOException {
        ...
        while (count > 0){
            ...
            connection.connect();
            logger.trace("Response: {}, {}", connection.getResponseCode(), connection.getResponseMessage());
        	if (connection.getResponseCode() != 200) {
        		try {
        			Thread.sleep(60000 + (int)(Math.random() * 30000)); // wait between 60 and 90 seconds
        		} catch (InterruptedException e1) {
        			logger.trace("Sleep has been interrupted");
        		}
        	} else {
        		return connection;
        	}
    ...
    public String getContentsFromConnection(URLConnection connection) {
        ...
        try {
        	...
        	throw new RuntimeException("Could not read contents from connection");
        }     	
